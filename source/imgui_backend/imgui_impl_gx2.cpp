...
// For allocating memory for GX2 in plugins/modules
...
void    ImGui_ImplGX2_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (bd->VertexBufferSize < vtx_buffer_size)
    {
        ...
        MEMFreeToMappedMemory(bd->VertexBuffer);
        bd->VertexBuffer = MEMAllocFromMappedMemoryForGX2Ex(vtx_buffer_size, GX2_VERTEX_BUFFER_ALIGNMENT);
    }
    if (bd->IndexBufferSize < idx_buffer_size)
    {
        bd->IndexBufferSize = idx_buffer_size;
        MEMFreeToMappedMemory(bd->IndexBuffer);
        bd->IndexBuffer = MEMAllocFromMappedMemoryForGX2Ex(idx_buffer_size, GX2_INDEX_BUFFER_ALIGNMENT);
    }
...
bool    ImGui_ImplGX2_CreateDeviceObjects()
{
    ...
    if (!WHBGfxLoadGFDShaderGroupMappedMem(bd->ShaderGroup, 0, shader_gsh))
    {
    ...
    if (!WHBGfxInitFetchShaderMappedMem(bd->ShaderGroup))
    {
        IM_DELETE(bd->ShaderGroup);
        return false;
    }
...
void    ImGui_ImplGX2_DestroyDeviceObjects()
{
    ...
    WHBGfxFreeShaderGroupMappedMem(bd->ShaderGroup);
    IM_DELETE(bd->ShaderGroup);
    bd->ShaderGroup = NULL;
